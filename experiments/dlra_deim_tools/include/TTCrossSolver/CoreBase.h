#ifndef MAT_SUBSET_EXPERIMENTS_CORE_BASE_H
#define MAT_SUBSET_EXPERIMENTS_CORE_BASE_H
...
/*!
 * @brief Shared implementation of a 3D core, logically of shape
 * \f$ r_0 \times n \times r_1 \f$ (left bond, mode, right bond), stored as its
 * left unfolding.
 * @tparam Scalar The underlying scalar type (e.g. `float`, `double`).
 *
 * The two concrete cores — `TensorTrainCore` (a TT-core, bonds are TT-ranks)
 * and `TensorFibersCore` (a sampled slab, bonds are fiber counts) — share
 * exactly the same storage and per-core linear algebra: unfolding access,
 * mode slices, factor absorption, QR/SVD sweep steps and DEIM/cross index
 * selection. Only the *combining* operations differ (`zip` and block-wise
 * `+`/Kronecker Hadamard for TT-cores, entry-wise algebra for slabs), so they
 * live on the children.
 *
 * This base is a plain implementation-sharing device, not a customization
 * point: there is no virtual dispatch, cores keep value semantics, and the
 * protected destructor prevents standalone or through-the-base use.
 *
 ...
 * neighbour.
 */
template <typename Scalar> class CoreBase {
  ...
  protected:
     ...
     */
    CoreBase(Eigen::Index r0, Eigen::Index n, Eigen::Index r1)
     ...
     */
    CoreBase(Eigen::MatrixX<Scalar> unfolding, Eigen::Index n)
        : left_rank(unfolding.rows() / n), mode(n),
          right_rank(unfolding.cols()), left_unfolding(std::move(unfolding)) {}

    // Implementation-sharing base only: never destroyed through the base.
    ~CoreBase() = default;
...
#endif // MAT_SUBSET_EXPERIMENTS_CORE_BASE_H
